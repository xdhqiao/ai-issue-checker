...
- `app/services/task_retry.py`：自动重试一次仍失败后的手动续跑，保留已完成文件。
...
3. `ReviewScheduler` 扫描 pending、允许进行唯一一次自动重试的 failed、以及 lease 过期的 running 任务，通过带旧状态和旧 lease 的 MongoDB 原子更新领取任务。正常执行时心跳持续续租，服务中断后才由新实例接管。
...
10. 自动重试一次仍失败的任务在管理页显示“继续确认”，点击后只重置未完成文件。

## 4. 真实 LLM 测试

测试日期：2026-07-23
模型：`deepseek-v4-flash`  
Task ID：`6a62106b6f850c3a0748a1f1`
...
- LLM API 调用：13 次；Control 文件 8 轮，Math 文件 5 轮。
- Prompt tokens：26,381。
- Completion tokens：3,125。
- Total tokens：29,506。
- LLM 累计耗时：64,035 ms。
- 任务墙钟处理时间：42,854 ms；两个文件并发处理，因此小于 LLM 累计耗时。
- 工具调用：19 次，全部成功；真实调用中没有已删除的 `file_read_diff`。
- 两个 CodeFile 的持久化 model trace 均记录模型 `deepseek-v4-flash`。

## 5. 结果链接

- [任务管理页](http://127.0.0.1:18080/admin/tasks.html)
- [本次真实 LLM 报告](http://127.0.0.1:18080/reports/polyspace-real-llm-demo/v1.html)
- [本次报告 JSON API](http://127.0.0.1:18080/api/reports/projects/polyspace-real-llm-demo/versions/v1)
