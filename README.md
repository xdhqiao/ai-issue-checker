...
当前系统保存并展示 LLM 返回的 `0~1` 置信度。若业务需要将置信度转换为明确结论，建议采用以下三级判断：

| confidence | 建议结论 |
|---:|---|
| `>= 0.8` | 确认为真实问题 |
| `0.2 ~ 0.8` | 证据不足，需要人工复核 |
| `< 0.2` | 大概率为 Polyspace 误报 |

因此，进行简单二元判断时，建议以 `confidence >= 0.8` 作为“真实问题”的阈值。需要注意，当前置信度是 LLM 基于源码证据给出的判断值，尚未经过统计概率校准，`0.8` 不代表严格意义上的 80% 正确率。`severity_color` 不参与置信度计算；红色问题可能是低置信度误报，橙色问题也可能是高置信度真实缺陷。对于 ECU 等安全敏感项目，建议使用人工确认过的历史 Issue 数据进一步校准阈值，并优先控制漏报率。
